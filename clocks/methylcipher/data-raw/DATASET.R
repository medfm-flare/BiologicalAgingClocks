...
load("data-raw/dataETOC2.Rd")
## this loads the CpG information
...
LeeRobust_CpGs <- data.frame(
  CpG = Lee_all$CpGs[Lee_all$Coefficient_RPC != 0][-1],
  coef = Lee_all$Coefficient_RPC[Lee_all$Coefficient_RPC != 0][-1]
)
usethis::use_data(LeeRobust_CpGs, overwrite = TRUE)
LeeRefinedRobust_CpGs <- data.frame(
  CpG = Lee_all$CpGs[Lee_all$Coefficient_refined_RPC != 0][-1],
...
LeeControl_CpGs <- data.frame(
  CpG = Lee_all$CpGs[Lee_all$Coefficient_CPC != 0][-1],
