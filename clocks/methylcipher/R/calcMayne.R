#' calcMayne
#'
#' @description A function to calculate the Mayne Gestational Age predictor
#'
...
calcMayne <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  ###################################################
  CpGCheck <- length(Mayne_CpGs$CpG) == sum(Mayne_CpGs$CpG %in% colnames(DNAm))
  ...
  if (CpGCheck == F && is.null(CpGImputation) && imputation == T) {
    ...
    present <- Mayne_CpGs$CpG %in% colnames(DNAm)

    betas <- DNAm[, na.omit(match(Mayne_CpGs$CpG, colnames(DNAm)))]
    tt <- rep(0, dim(DNAm)[1])

    tt <- rowSums(sweep(as.matrix(betas), MARGIN = 2, Mayne_CpGs$coef[present], `*`), na.rm = T) + 24.99026
    ...
    missingCpGs <- Mayne_CpGs$CpG[!(Mayne_CpGs$CpG %in% colnames(DNAm))]
    ...
    betas <- DNAm[, match(Mayne_CpGs$CpG, colnames(DNAm))]
    tt <- rep(0, dim(DNAm)[1])
    tt <- rowSums(sweep(betas, MARGIN = 2, Mayne_CpGs$coef, `*`)) + 24.99026
