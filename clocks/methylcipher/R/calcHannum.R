#' calcHannum
#'
#' @description A function to calculate the Hannum epigenetic clock
#'
...
#' @export
#'
#' @examples calcHannum(exampleBetas, examplePheno, imputation = T)
calcHannum <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  #######################
  ### Read in the Data###
  #######################
  ...
  if (CpGCheck == F && is.null(CpGImputation) && imputation == T) {
    ...
    tt <- rowSums(sweep(betas, MARGIN = 2, Hannum_CpGs$Coefficient, `*`))
