#' calcLeeRobust
#'
#' @description A function to calculate the Robust Predictor of Placental Age (Lee)
#'
...
calcLeeRobust <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  ###################################################
  CpGCheck <- length(LeeRobust_CpGs$CpG) == sum(LeeRobust_CpGs$CpG %in% colnames(DNAm))
