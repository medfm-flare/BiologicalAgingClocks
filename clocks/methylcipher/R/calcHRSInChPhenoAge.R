...
calcHRSInChPhenoAge <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  # data("HRSInCHPhenoAge_CpGs")

  ###################################################
  ### Check if all necessary CpGs are in the data ###
  ###################################################
  CpGCheck <- length(HRSInCHPhenoAge_CpGs$CpG) == sum(HRSInCHPhenoAge_CpGs$CpG %in% colnames(DNAm))
  ...
  if (CpGCheck == F && is.null(CpGImputation) && imputation == T) {
    ...
    present <- HRSInCHPhenoAge_CpGs$CpG %in% colnames(DNAm)

    betas <- DNAm[, na.omit(match(HRSInCHPhenoAge_CpGs$CpG, colnames(DNAm)))]
    tt <- rep(0, dim(DNAm)[1])

    tt <- rowSums(sweep(as.matrix(betas), MARGIN = 2, HRSInCHPhenoAge_CpGs$Weight[present], `*`), na.rm = T) + 52.8334080
    ...
    missingCpGs <- HRSInCHPhenoAge_CpGs$CpG[!(HRSInCHPhenoAge_CpGs$CpG %in% colnames(DNAm))]
    ...
    betas <- DNAm[, match(HRSInCHPhenoAge_CpGs$CpG, colnames(DNAm))]
    tt <- rep(0, dim(DNAm)[1])
    tt <- rowSums(sweep(betas, MARGIN = 2, HRSInCHPhenoAge_CpGs$Weight, `*`)) + 52.8334080
