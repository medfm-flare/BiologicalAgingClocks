#' calcLeeControl
#'
#' @description A function to calculate the Predictor of Control Placental Age (Lee)
#'
...
calcLeeControl <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  ###################################################
  CpGCheck <- length(LeeControl_CpGs$CpG) == sum(LeeControl_CpGs$CpG %in% colnames(DNAm))
