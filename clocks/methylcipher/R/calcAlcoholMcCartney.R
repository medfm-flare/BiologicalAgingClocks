...
#'
#' @description A function to calculate the McCartney predictor of Alcohol
#'
...
calcAlcoholMcCartney <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  CpGCheck <- length(Alcohol_CpGs$CpG) == sum(Alcohol_CpGs$CpG %in% colnames(DNAm))
  ...
  if (CpGCheck == F && is.null(CpGImputation) && imputation == T) {
    ...
    present <- Alcohol_CpGs$CpG %in% colnames(DNAm)

    betas <- DNAm[, na.omit(match(Alcohol_CpGs$CpG, colnames(DNAm)))]

    tt <- rowSums(sweep(as.matrix(betas), MARGIN = 2, Alcohol_CpGs$Beta[present], `*`), na.rm = T)
    ...
    message("Imputation of mean CpG Values occured for Alcohol_McCartney")
    missingCpGs <- Alcohol_CpGs$CpG[!(Alcohol_CpGs$CpG %in% colnames(DNAm))]
    ...
    betas <- DNAm[, match(Alcohol_CpGs$CpG, colnames(DNAm))]

    tt <- rowSums(sweep(betas, MARGIN = 2, Alcohol_CpGs$Beta, `*`))
