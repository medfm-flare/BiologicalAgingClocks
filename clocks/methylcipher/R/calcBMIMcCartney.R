#' calcBMIMcCartney
#'
#' @description A function to calculate the McCartney predictor of BMI
#'
...
calcBMIMcCartney <- function(DNAm, pheno = NULL, CpGImputation = NULL, imputation = T) {
  ...
  CpGCheck <- length(BMI_CpGs$CpG) == sum(BMI_CpGs$CpG %in% colnames(DNAm))
  ...
  if (CpGCheck == F && is.null(CpGImputation) && imputation == T) {
    ...
    present <- BMI_CpGs$CpG %in% colnames(DNAm)

    betas <- DNAm[, na.omit(match(BMI_CpGs$CpG, colnames(DNAm)))]

    tt <- rep(0, dim(DNAm)[1])

    tt <- rowSums(sweep(as.matrix(betas), MARGIN = 2, BMI_CpGs$Beta[present], `*`), na.rm = T)
    ...
    message("Imputation of mean CpG Values occured for BMI_McCartney")
    missingCpGs <- BMI_CpGs$CpG[!(BMI_CpGs$CpG %in% colnames(DNAm))]
    ...
    betas <- DNAm[, match(BMI_CpGs$CpG, colnames(DNAm))]

    tt <- rowSums(sweep(betas, MARGIN = 2, BMI_CpGs$Beta, `*`))
