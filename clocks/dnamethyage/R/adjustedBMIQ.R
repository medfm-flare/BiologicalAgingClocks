...
BMIQcalibration <- function(datM, goldstandard.beta, nL=3, doH=TRUE, nfit=20000, th1.v=c(0.2,0.75), th2.v=NULL,
    niter=5, tol=0.001, plots=FALSE, calibrateUnitInterval=TRUE, use_cores=detectCores()){
...
#    datM[ii,] <- process_S(beta2.v)
#}
if(Sys.info()["sysname"] != "Linux"){
    use_cores <- 1
}
sample_id <- rownames(datM)
probes <- colnames(datM)
if (use_cores == 1 ){
    datM <- apply(datM, 1, function(x) process_S(as.numeric(x)))
}else{
    #datM <- data.frame(lapply(data.frame(t(datM)), function(x) process_S(as.numeric(x))))
    datM <- data.frame(mclapply(data.frame(t(datM)), function(x) process_S(as.numeric(x)), mc.cores=use_cores))
}
rownames(datM) <- probes
colnames(datM) <- sample_id
